...
def _seed_accounts(db: Session) -> None:
    ...
    users = ["fazrin", "magfira"]
    for user_id in users:
        for suffix, name, atype in account_templates:
            aid = f"{user_id}_{suffix}"
            if not db.query(Account).filter(Account.id == aid).first():
                db.add(Account(id=aid, display_name=name, type=atype, owner_id=user_id))
