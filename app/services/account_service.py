...
def list_accounts(db: Session, active_only: bool = True) -> list[Account]:
    ...
    return q.order_by(Account.display_name).all()
...
def create_account(db: Session, account_id: str, display_name: str, acct_type: str, currency: str = "IDR") -> Account:
    acct = Account(id=account_id, display_name=display_name, type=acct_type, currency=currency)
    db.add(acct)
    db.commit()
    db.refresh(acct)
    return acct
...
def compute_balances(db: Session) -> list[AccountBalance]:
    """Compute current balance for every active account from the transaction ledger."""
    accounts = list_accounts(db, active_only=True)
    result: list[AccountBalance] = []

    for acct in accounts:
        balance = _compute_account_balance(db, acct.id)
        result.append(AccountBalance(account_id=acct.id, display_name=acct.display_name, balance=balance))
